	...
	*/
	...
	while (!disp.is_closed() && !disp.is_keyESC() && !disp.is_keyQ()) {
		...
		if (disp.button() & 1) { // Left button clicked.
			int i = floor(18.0 * disp.mouse_y()/ (double)disp.height());		//ok c'estcarrement dingo depuis le debut y'en avait 18..
			int j = floor(9.0* disp.mouse_x() / (double)disp.width());
			...
			if (table[i][j] == 0) {
				...
				if (player == 1) {
					...
					//scene.draw_image(j * 100, i * 100, 0, 0, image, 1);			//applique en permanance au dessus en couches // comment réduire taille ?
